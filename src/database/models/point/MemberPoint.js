'use strict'; // eslint-disable-line strict, lines-around-directive

import { Model } from 'sequelize';
import { field } from '../..';

export default (sequelize, DataTypes) => {
	class MemberPoint extends Model {
		static associate(models) {
			MemberPoint.belongsTo(models.Member, { as: 'member', foreignKey: 'member_id', targetKey: 'id', constraints: false });
			MemberPoint.belongsTo(models.Project, { as: 'project', foreignKey: 'project_id', targetKey: 'id', constraints: false });
		}
	}

	MemberPoint.init(
		// fields
		{
			id: field(DataTypes.BIGINT.UNSIGNED, 'id', false, { unique: true, primaryKey: true, autoIncrement: true }),
			type_cd: field(DataTypes.STRING(64), '포인트 출처(코드)'),
			member_id: field(DataTypes.BIGINT.UNSIGNED, 'members.id', false),
			project_id: field(DataTypes.BIGINT.UNSIGNED, 'projects.id'),
			job_id: field(DataTypes.BIGINT.UNSIGNED, 'jobs.id'),
			point: field(DataTypes.INTEGER(16), '포인트'),
			payment_time: field(DataTypes.DATE, '지급 일시'),
		},
		// options
		{
			timestamps: true,
			createdAt: 'created_at',
			updatedAt: 'updated_at',
			tableName: 'usr_member_point',
			comment: '일반회원 포인트정보',
			indexes: [],
			sequelize,
		},
	);

	return MemberPoint;
};
